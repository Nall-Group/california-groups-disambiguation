...
- **`gaps_needs_audit.txt`** = the work queue, sorted by bill count (priority). One org per line.
  Whatever is in this file is NOT yet triaged. Currently: **42,510 remaining**.
- Each completed wave removes its orgs from this file (the cursor advances). When it hits 0, done.
- **Next wave number to use: 52.** (Waves 1–51 are committed. Wave numbers only label the temp
...
- Done: bulk pass (tasks 1224–1226), 112 acronym matches (1242), wave 1 (1239–1241),
  waves 2–51 → masters above. ~25,300 of 67,609 orgs triaged (~37%).
- Remaining: 42,510 in `gaps_needs_audit.txt`.
