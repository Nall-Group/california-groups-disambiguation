...
`org_names_in_crosswalk.csv` (a redundant CSV mirror of every name already in the crosswalk JSON)
was **retired 2026-07-09**. It violated this folder's core invariant — a string is either in the
crosswalk **or** in exactly one CSV here, never both. `scripts/regenerate_org_subsets.py` now
simply **drops** any routing-CSV name it finds already present in the crosswalk (rather than
mirroring it into a second file), and `extract_org_names.py` checks membership directly against the
crosswalk JSON.
